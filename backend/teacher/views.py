...
class Form(forms.ModelForm):
    ...
    class Meta:
        model = MODEL
        fields = ["name", "classes", "subject", "phoneNo",
                  "email", "password", "prn", "description"]
...
class Form2(forms.ModelForm):
    ...
    class Meta:
        model = MODEL
        fields = ["name", "classes", "subject",
                  "phoneNo", "email", "prn", "description"]
...
def add(request):
    try:
        print(Sitesetting.objects.get(key="admin_email").value)
        form = Form()
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                send_mail(
                    subject="Django | Your Password",
                    message="Class Shedule Accepted",
                    from_email=Sitesetting.objects.get(key="admin_email").value,
                    recipient_list=[request.POST.get('email')],
                    html_message="Your Password is :.<b>"+str(request.POST.get('password'))+"</b>",
                )
