  ...
  $.extend({
    notifier: function(options){
      ...
      if(parameters.positionY=='top' && parameters.positionX=='right'){
        // positions = 'top:50px;right:10px';
        if(parameters.scrollPosition != 'UnDefined'){  
          var ElementPosition  =  Math.round(parameters.scrollPosition);
          positions = "top:"+ElementPosition+"px;right:10px;";
        }else{
          positions = 'top:50px;right:10px';
        }
        ...
        if(parameters.scrollPosition != 'UnDefined'){  
          var ElementPosition  =  Math.round(parameters.scrollPosition);
          positions = "top:"+ElementPosition+"px;left:10px";
        }else{
          positions = 'top:17px;left:10px';
        }
        pos = 'tl';
        ...
        if(parameters.scrollPosition != 'UnDefined'){  
          var ElementPosition  =  Math.round(parameters.scrollPosition);
          positions = "bottom:"+ElementPosition+"px;right:10px;";
        }else{
          positions = 'bottom:10px;right:10px';
        }
        pos = 'br';
        ...
        if(parameters.scrollPosition != 'UnDefined'){  
          var ElementPosition  =  Math.round(parameters.scrollPosition);
          positions = "bottom:"+ElementPosition+"px;left:10px;";
        }else{
          positions = 'bottom:10px;left:10px';
        }
        pos = 'bl';
      ...
      $('#notifier').append('<li id="box'+ id +'" class="notif '+ parameters.type +'"><div class="row"><div class="cols-sm-4 icon"></div><div class="cols-sm-8"><div class="text"><h5>'+ parameters.title.substring(0,30) +'</h5><p>'+ parameters.text.substring(0,100) +'</p></div><div class="close" data-id="'+ id +'"></div></div></div></li>');
